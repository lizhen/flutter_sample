...
class SignaturePainter extends CustomPainter {
  ...
  final List<Offset>
      points; //point[0] touchDown offset, point[1] touchMoveOffset  point[2] clip square start offset, point[3],image draw size
...
class _ImageCutPageState extends State<ImageCutPage> {
        ...
        children: [
          ...
          CustomPaint(
            painter: new SignaturePainter(
                _points, width, height, cWidth, cHeight, image),
          ),
          Container(
            child: Row(
              children: <Widget>[
                RaisedButton(
                  ...
                  onPressed: () async {
                    ...
                    var source = Rect.fromLTWH(
                        _points[2].dx * rate,
                        (_points[2].dy - (height - _points[3].dy) / 2) * rate,
                        cWidth * rate,
                        cHeight * rate);
                    var dest =
                        Rect.fromLTWH(0, 0, cWidth * rate, cHeight * rate);
  ...
  onPanDown(DragDownDetails details) {
    ...
    setState(() {
      ...
      dWidth = cWidth;
      double radius = 20;
      if (hitPoint(Offset(_points[2].dx + cWidth, _points[2].dy + cHeight),
          radius, localPosition)) {
        downPosition = DownPosition.RIGHT_DOWN;
        isDrag = false;
      } else if (hitPoint(Offset(_points[2].dx + cWidth, _points[2].dy), radius,
          localPosition)) {
        downPosition = DownPosition.RIGHT_UP;
        isDrag = false;
      } else if (hitPoint(Offset(_points[2].dx, _points[2].dy + cHeight),
          radius, localPosition)) {
